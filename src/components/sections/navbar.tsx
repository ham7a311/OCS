...
// Stable module-level reference so the observer is not rebuilt every render.
...
export function Navbar() {
  ...
  // While the panel is open it owns the screen: page scroll is frozen and
  // Escape closes it.
  useEffect(() => {
    if (!menuOpen) return;
  ...
  return (
    <>
          ...
          >
            ...
            <div className="flex shrink-0 items-center gap-0.5 sm:gap-2">
              ...
              <button
                ...
                className="grid size-11 place-items-center rounded-md text-ink-muted transition-colors duration-200 ease-ui hover:bg-surface-2 hover:text-ink lg:hidden"
              >
                {menuOpen ? (
      ...
      {/* Mobile navigation is composed for the phone rather than being the
          desktop bar folded up: large type, numbered, one clear action. */}
      <AnimatePresence>
        {menuOpen ? (
          <motion.div
            id="mobile-menu"
