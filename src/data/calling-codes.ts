...
const RAW_CALLING_CODES: CallingCode[] = [
...
export const CALLING_CODES: CallingCode[] = RAW_CALLING_CODES.filter((entry) => entry.iso2 !== "IL").sort(
  (a, b) => a.name.localeCompare(b.name, "en"),
);

export const DEFAULT_CALLING_CODE =
  CALLING_CODES.find((entry) => entry.iso2 === "OM") ?? CALLING_CODES[0]!;

const BY_ISO = new Map(CALLING_CODES.map((entry) => [entry.iso2, entry]));

const BY_DIAL_LONGEST = [...CALLING_CODES].sort((a, b) => b.dial.length - a.dial.length);

export function callingCodeFlagSrc(iso2: string) {
  return `https://flagcdn.com/w40/${iso2.toLowerCase()}.png`;
}

export function findCallingCodeByIso(iso2: string) {
  return BY_ISO.get(iso2.toUpperCase());
}

export function findCallingCodeByDialDigits(digits: string) {
  return BY_DIAL_LONGEST.find((entry) => digits.startsWith(entry.dial));
}
...
export function composePhone(country: CallingCode, national: string) {
...
export function isValidStoredPhone(value: string) {
  ...
  const country = findCallingCodeByDialDigits(digits);
  if (!country) return false;
  const national = digits.slice(country.dial.length);
  return national.length >= 4;
}

export function parsePhone(value: string): { country: CallingCode; national: string } {
  const trimmed = value.trim();
  if (!trimmed) {
    return { country: DEFAULT_CALLING_CODE, national: "" };
  }

  let digits = digitsOnly(trimmed);
  if (digits.startsWith("00")) digits = digits.slice(2);

  const matched = findCallingCodeByDialDigits(digits);
  if (matched) {
    return { country: matched, national: digits.slice(matched.dial.length) };
  }

  if (digits.startsWith("0")) digits = digits.slice(1);
  return { country: DEFAULT_CALLING_CODE, national: digits };
}
